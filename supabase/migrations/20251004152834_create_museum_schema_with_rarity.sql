/*
  # Musée des Civilisations Noires - Database Schema avec système de rareté
  ...
  1. New Tables
    - `artworks`
      ...
      - `rarity` (text) - Rareté: common, rare, legendary
    ...
    - `visitor_passport`
      ...
      - `card_collected` (boolean) - Carte collectée ou non
  ...
  2. Security
    ...
    - Anonymous user access for visitor passport (using device-generated user_id)
*/
...
-- Create artworks table with rarity
...
-- Visitor passport policies (anonymous access allowed)
CREATE POLICY "Anyone can view visitor passport entries"
  ON visitor_passport FOR SELECT
  TO anon, authenticated
  USING (true);

CREATE POLICY "Anyone can insert visitor passport entries"
  ON visitor_passport FOR INSERT
  TO anon, authenticated
  WITH CHECK (true);

CREATE POLICY "Anyone can update visitor passport entries"
  ON visitor_passport FOR UPDATE
  TO anon, authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Anyone can delete visitor passport entries"
...
-- Insert sample artworks with rarity
...
(
  ...
  'XIVe siècle',
  'Mali',
  'Ce masque rituel dogon représente l''une des pièces les plus rares de notre collection. Les sculptures complexes et les motifs géométriques gravés racontent l''histoire de la création selon la mythologie dogon, où chaque ligne et chaque courbe possède une signification sacrée.',
  'This Dogon ritual mask represents one of the rarest pieces in our collection. Complex sculptures and carved geometric patterns tell the story of creation according to Dogon mythology.',
  'MCN001',
  'legendary',
